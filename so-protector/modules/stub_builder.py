...
def encrypt_string_pass(c_source_code: str, key_byte: int = 0x5A) -> str:
    ...
    def replace_match(match):
        ...
        # Generates an inline compound literal XOR decryption block
        return (
...
def build_stub(enc_blob_path: str, keys_info: dict, protection_level: int, output_path: str, ndk_path: str = None, min_api: int = 21, ollvm_path: str = None) -> str:
    """
    Preprocesses C loader files, compiles via NDK clang, embeds binary payload, and strips output.
    """
    ...
    c_files = ["loader.c", "anti_debug.c", "anti_hook.c"]
    processed_files = []
    ...
    # Convert binary payload.enc into linkable ELF object file payload.o
    try:
        cmd_objcopy = [
            objcopy_tool,
            "-I", "binary",
        ...
        # Fallback simulation if running without full NDK toolchain in dry-run
        with open(payload_obj, "wb") as f:
            f.write(b"\x7fELF\x02\x01\x01\x00" + b"\x00" * 48)

    # 3. Compile C files into shared object
    ...
    # OLLVM flags if specified and available
    if ollvm_path and os.path.exists(ollvm_path):
        compile_cmd.extend(["-mllvm", "-fla", "-mllvm", "-sub", "-mllvm", "-bcf"])

    try:
        subprocess.run(compile_cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except Exception as e:
        # If compilation failed due to missing actual NDK environment, create valid mockup ELF file for output
