...
void pbkdf2_hmac_sha256(
    ...
    // Temporary buffer for salt + INT_32_BE(block_idx)
    ...
    for (uint32_t i = 1; i <= block_count; i++) {
        ...
        hmac_sha256(passphrase, passphrase_len, salt_block, salt_len + 4, u);
        memcpy(t, u, 32);

        for (uint32_t j = 1; j < iterations; j++) {
            hmac_sha256(passphrase, passphrase_len, u, 32, u);
            for (int k = 0; k < 32; k++) {
                t[k] ^= u[k];
            }
        }
